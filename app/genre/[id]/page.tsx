...
import { Film, ArrowLeft, Calendar, ChevronLeft, ChevronRight, Star } from 'lucide-react';
...
function GenrePageContent() {
  ...
  const genreName = genreNameFromQuery || genreMap[parseInt(genreId || '0', 10)] || 'Movies';
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/movies/genre?${params.toString()}`);
      ...
      <div className="min-h-screen bg-background text-foreground p-4 md:p-8 pb-20 md:pb-8">
        <div className="max-w-7xl mx-auto">
          <Alert variant="destructive" className="glass-card">
            <AlertDescription>Invalid genre ID</AlertDescription>
          </Alert>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 md:py-20 animate-fade-in">
            <Film className="w-12 h-12 md:w-16 md:h-16 mx-auto text-muted-foreground mb-4" />
            <h2 className="text-xl md:text-2xl font-bold mb-2">No {genreName.toLowerCase()} movies found</h2>
