...
import type { BuyStockResponse } from '@/lib/market/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Using buy_stock_with_user which accepts user_id parameter
    // This works with Firebase Auth. Once Supabase Auth is set up,
    // you can switch to buy_stock() which uses auth.uid()
    
    const supabase = createServerClient();
    
    const { data, error } = await supabase.rpc('buy_stock_with_user', {
    ...
    const result = data as BuyStockResponse;
