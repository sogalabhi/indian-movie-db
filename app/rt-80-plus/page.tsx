...
import { Award, ArrowLeft, Calendar, ChevronLeft, ChevronRight, Star } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        console.error('Error fetching RT 80%+ movies:', err);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
                    ...
                    <Link href={`/movie/${movie.id}`} className="block h-full">
                      <Card className="glass-card overflow-hidden hover-scale cursor-pointer group h-full flex flex-col">
                        <div className="relative h-[210px] md:h-[270px] lg:h-[300px] w-full overflow-hidden">
                          ...
                          {/* RT Rating Badge */}
                          {movie.rtRating && (
                            <Badge className="absolute top-2 left-2 glass-card text-xs md:text-sm px-2 py-1 bg-red-500/20 text-red-500 border-red-500/30">
          ...
          <div className="text-center py-12 md:py-20 animate-fade-in">
            <Award className="w-12 h-12 md:w-16 md:h-16 mx-auto text-muted-foreground mb-4" />
            <h2 className="text-xl md:text-2xl font-bold mb-2">No Rotten Tomatoes 80%+ movies found</h2>
...
export default function Rt80PlusPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-background text-foreground p-4 md:p-8 pb-20 md:pb-8">
        <div className="max-w-7xl mx-auto">
